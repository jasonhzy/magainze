...
<body>
	<div class="member_top">
		...
		<div class="m_t_font">所有杂志(<font color=red><?=isset($magainze_total)? $magainze_total : '0' ?></font>)</div>
		<?php if (isset($verify) && $verify == '0') :?>
			<div class="m_t_font">待审核(<?=$total ?>)</div>
		<?php else:?>
			<a href="<?=base_url()."admin/magainze/index?verify=0" ?>"><div class="m_t_btns">待审核</div></a>
		<?php endif;?>
		<?php if (isset($verify) && $verify == '2') :?>
			<div class="m_t_font">未通过(<?=$total ?>)</div>
		<?php else:?>
			<a href="<?=base_url()."admin/magainze/index?verify=2" ?>"><div class="m_t_btns">未通过</div></a>
		<?php endif;?>
		<?php if (isset($verify) && $verify == '1') :?>
			<div class="m_t_font">通过(<?=$total ?>)</div>
		<?php else:?>
			<a href="<?=base_url()."admin/magainze/index?verify=1" ?>"><div class="m_t_btns">通过</div></a>
		<?php endif;?>
		<a href="<?=base_url()."admin/magainze/magainze_add" ?>"><div class="m_t_btns">新增</div></a>
	</div>
	...
	<div class="member_list">
		<table cellpadding="10" cellspacing="0">
			...
			<tr>
					...
					<div class="magaimg_font">
						<?php if(isset($v['COVER_URL'])&&$v['COVER_URL']) :?>
							<a id="magainze_image_url" href="<?=$v['COVER_URL']?>" >
								<img src="<?=$v['COVER_URL'] ? $v['COVER_URL'] : ''?>" class="magazineimg"/>
							</a>
							<br/>
						<?php endif;?>
						<?=$v['NAME']?>
				...
				<td class="m_l_t_l">
					<?php if(!$v['VERIFY']):?>
	                    <a href="<?php echo site_url('admin/magainze/verify_magainze'); ?>?verify=1&verify_id=<?=$v['ID']?>">通过</a>&nbsp;
                	<?php endif;?>
                    <a href="<?php echo site_url('admin/magainze/magainze_edit'); ?>?modify_id=<?=$v['ID']?>">修改</a>&nbsp;
                    <a href="<?php echo site_url('admin/magainze/delete_magainze');?>?delete_id=<?=$v['ID']?>" onclick="return confirm('确认删除?')" >删除</a>
				</td>
			</tr>
			 <?php endforeach;?>
		    <?php endif;?>
			<tr>
				<td class="table_page" colspan="8">
					<div class="m_l_t_dtbn">
						...
						<input type="hidden" name="del_url" value="<?php echo base_url().'admin/magainze/batchDel';?>"></input>
					</div>
...
<script>
	$(document).ready(function(){
		...
		$('#member_search').bind('click', function(){
			...
			window.location.href = '<?=base_url()."admin/magainze/index"?>' + strParams;
		});
	});
</script>
</html>
