...
<body>
	<div class="member_top">
		...
		<?php else:?>
			<a href="<?=base_url()."admin/magainze/publish?publish=0" ?>"><div class="m_t_btns">屏蔽</div></a>
		<?php endif;?>
		<?php if ($publish == '1') :?>
			<div class="m_t_font">发布(<?=$total ?>)</div>
		<?php else:?>
			<a href="<?=base_url()."admin/magainze/publish?publish=1" ?>"><div class="m_t_btns">发布</div></a>
		<?php endif;?>
	</div>
	...
	<div class="member_list">
		<table cellpadding="10" cellspacing="0">
			...
			<tr>
				...
				<td class="m_l_t_l">
					<a href="<?php echo site_url('admin/magainze/publish_magainze'); ?>?page_type=publish_page&publish=<?=$v['PUBLISH'] == 1 ? '0' : '1'?>&publish_id=<?=$v['ID']?>">
                		<?=$v['PUBLISH'] == 1 ? '屏蔽' : '发布'?>
                	</a>
				</td>
			</tr>
...
<script>
	$(document).ready(function(){
		...
		$('#member_search').bind('click', function(){
			...
			window.location.href = '<?=base_url()."admin/magainze/publish"?>' + strParams;
		});
	});
</script>
</html>