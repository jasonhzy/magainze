...
<body>
	<div class="member_top">
		...
		<?php else:?>
			<a href="<?=base_url()."admin/article/verify?verify=2" ?>"><div class="m_t_btns">未通过</div></a>
		<?php endif;?>
		<?php if ($verify == '1') :?>
			<div class="m_t_font">通过(<?=$total ?>)</div>
		<?php else:?>
			<a href="<?=base_url()."admin/article/verify?verify=1" ?>"><div class="m_t_btns">通过</div></a>
		<?php endif;?>
	</div>
		...
		<table cellpadding="10" cellspacing="0">
			...
			<?php foreach($article as $key=>$v):?>
	            <tr>
					...
					<td class="m_l_t_l">
		                <a href="<?php echo site_url('admin/article/verify_article'); ?>?page_type=verify_page&verify=<?=$v['VERIFY'] == 1 ? '2' : '1'?>&verify_id=<?=$v['ID']?>">
	                		<?=$v['VERIFY'] == 1 ? '未通过' : '通过'?>
	                	</a>
					</td>
	            </tr>
...
<script>
	$(document).ready(function(){
		$('#article_search').bind('click', function(){
			...
			window.location.href = '<?=base_url()."admin/article/verify"?>' + strParams;
		});
	});
</script>
</html>